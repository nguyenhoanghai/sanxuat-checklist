...
GPRO.CheckList = function () {
    ...
    var RegisterEvent = function () {
        ...
        $('[findExcel]').click(function () {
            window.location.href = Global.UrlAction.Excel + "?start=" + $("#J_start").val() + "&&end=" + $("#J_end").val() + '&&clType=' + $('#cl_type').val() + '&&Organ=' + $('#organ').val()+'&&IsTracuu=true';
        });
    ...
    function GetAllJob() {
        $.ajax({
            ...
            type: 'post',
            data: JSON.stringify({ 'start': new Date(), 'end': new Date(), 'clType': $('#cl_type').val(), 'Organ': $('#organ').val(),'IsTracuu':true }),
    ...
    function DrawTable(objs) {
        ...
        if (objs != null && objs.length > 0) {
            ...
            $.each(objs, function (i, item) {
                ...
                if (notSem || groupID != item.PartOfOrganId) {
