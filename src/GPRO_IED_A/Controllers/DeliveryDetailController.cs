...
namespace SanXuatCheckList.Controllers
{
    public class DeliveryDetailController : BaseController
    {
        ...
        public JsonResult Gets(int Id, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = "")
        {
            try
            {
                var objs = BLLDeliveryDetail.Instance.GetList(Id, jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult Save( DeliveryDetailModel obj)
        {
            ...
            try
            {
                ...
                rs = BLLDeliveryDetail.Instance.CreateOrUpdate(obj);
                if (!rs.IsSuccess)
                {
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                result = BLLDeliveryDetail.Instance.Delete(Id, UserContext.UserID);
                if (!result.IsSuccess)
                {
                    JsonDataResult.Result = "ERROR";
        ...
        public void ExportToExcel(int deliveryId)
        {
            ...
            {
                #region get data
                var objModel = BLLDeliveryDetail.Instance.GetDeliveryDetails(deliveryId);
