    ...
    public class AccessoryTypeController : BaseController
    {
        ...
        public JsonResult Gets(string keyword, int jtStartIndex = 0, int jtPageSize = 1000, string jtSorting = "")
        {
            try
            {
                if (isAuthenticate)
                {
                    var objs = BLLAccessoryType.Instance.GetList(keyword, jtStartIndex, jtPageSize, jtSorting);
        ...
        [HttpPost]
        public JsonResult Save(AccessoryTypeModel model)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    ...
                    responseResult = BLLAccessoryType.Instance.InsertOrUpdate(model, isOwner);
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    result = BLLAccessoryType.Instance.Delete(Id, UserContext.UserID, isOwner);
                    if (!result.IsSuccess)
                    {
                        JsonDataResult.Result = "ERROR";
        ...
        public JsonResult GetSelectList()
        {
            ...
            {
                JsonDataResult.Result = "OK";
                JsonDataResult.Data = BLLAccessoryType.Instance.GetSelectItem();
            }
            catch (Exception ex)
            {
                //add error
