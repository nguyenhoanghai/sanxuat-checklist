    ...
    public class SizeController : BaseController
    {
        ...
        public JsonResult Gets(string keyword, int jtStartIndex = 0, int jtPageSize = 1000, string jtSorting = "")
        {
            try
            {
                if (isAuthenticate)
                {
                    var objs = BLLSize.Instance.GetList(keyword, jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult Save(SizeModel model)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    ...
                    responseResult = BLLSize.Instance.InsertOrUpdate(model, isOwner);
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    result = BLLSize.Instance.Delete(Id, UserContext.UserID, isOwner);
                    if (!result.IsSuccess)
                    {
                        JsonDataResult.Result = "ERROR";
        ...
        public JsonResult GetSelectList()
        {
            ...
            {
                JsonDataResult.Result = "OK";
                JsonDataResult.Data = BLLSize.Instance.GetSelectItem();
            }
            catch (Exception ex)
            {
                //add error
