    ...
    public class StatusTypeController : BaseController
    {
        ...
        public JsonResult Gets(string keyword, int jtStartIndex = 0, int jtPageSize = 1000, string jtSorting = "")
        {
            try
            {
                if (isAuthenticate)
                {
                    var objs = BLLStatusType.Instance.GetList(keyword, jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult Save(StatusTypeModel model)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    ...
                    responseResult = BLLStatusType.Instance.InsertOrUpdate(model, isOwner);
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    result = BLLStatusType.Instance.Delete(Id, UserContext.UserID, isOwner);
                    if (!result.IsSuccess)
                    {
                        JsonDataResult.Result = "ERROR";
        ...
        public JsonResult GetSelectList()
        {
            ...
            {
                JsonDataResult.Result = "OK";
                JsonDataResult.Data = BLLStatusType.Instance.GetSelectItem();
            }
            catch (Exception ex)
            {
                //add error
