    ...
    public class UnitTypeController : BaseController
    {
        ...
        public JsonResult Gets(string keyword,  int jtStartIndex = 0, int jtPageSize = 1000, string jtSorting = "")
        {
            try
            {
                if (isAuthenticate)
                { 
                    var objs = BLLUnitType.Instance.GetList(keyword,jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult Save(UnitTypeModel model)
        {
            ...
            try
            {
                ...
                { 
                    ...
                    responseResult = BLLUnitType.Instance.InsertOrUpdate(model, isOwner);
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    result = BLLUnitType.Instance.Delete(Id, UserContext.UserID, isOwner);
                    if (!result.IsSuccess)
                    {
                        JsonDataResult.Result = "ERROR";
        ...
        public JsonResult GetSelectList()
        {
            ...
            {
                JsonDataResult.Result = "OK";
                JsonDataResult.Data = BLLUnitType.Instance.GetSelectItem( );
            }
            catch (Exception ex)
            {
                //add error
