    ...
    public class UnitController : BaseController
    {
        ...
        public JsonResult Gets(int typeId, int jtStartIndex = 0, int jtPageSize = 1000, string jtSorting = "")
        {
            try
            {
                if (isAuthenticate)
                {
                    var objs = BLLUnit.Instance.GetList(typeId, jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult Save(UnitModel model)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    ...
                    responseResult = BLLUnit.Instance.InsertOrUpdate(model, isOwner);
        ...
        public JsonResult Delete(int Id)
        {
            ...
            try
            {
                if (isAuthenticate)
                {
                    result = BLLUnit.Instance.Delete(Id, UserContext.UserID, isOwner);
                    if (!result.IsSuccess)
                    {
                        JsonDataResult.Result = "ERROR";
        ...
        public JsonResult GetSelectList(string typeCode)
        {
            ...
            {
                JsonDataResult.Result = "OK";
                JsonDataResult.Data = BLLUnit.Instance.GetSelectItem(typeCode);
            }
            catch (Exception ex)
            {
                //add error
